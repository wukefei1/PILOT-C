...
def process_file_geolife(ff, dir_name):
    ...
    for line in all_data:
        ...
        h = np.float64(line[3])
        t = round(np.float64(line[4]) * 86400)
...
def process_file_geolife_debug(ff, dir_name):
    all_data = []
    file_names = os.listdir(ff)
    file_names.sort()
    ...
    traj_path = os.path.join('/home/wkf/data/PRESS/datasets/geolife_debug', f"traj_{dir_name}.txt")
    with open(traj_path, "w") as f:
        for j in range(len(traj)):
            f.write(f"{t_list[j]},{traj[j][0]},{traj[j][1]}\n")
    print(ff, flush=True)
...
def preprocess_geolife_debug():
    if os.path.exists('/home/wkf/data/PRESS/datasets/geolife_debug'):
        os.system('rm -rf /home/wkf/data/PRESS/datasets/geolife_debug')
        
    os.makedirs('/home/wkf/data/PRESS/datasets/geolife_debug')
    ...
    with multiprocessing.Pool() as pool:
        results = []
        for ff, dir_name in tot_path:
            result = pool.apply_async(process_file_geolife_debug, args=(ff, dir_name,))
            results.append(result)
        tot_cnt = 0
        for result in results:
            cnt = result.get()
    ...
    print("preprocess geolife_debug done")

preprocess_geolife_debug()